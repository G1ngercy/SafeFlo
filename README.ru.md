...
SafeFlo не использует никаких глобальных путей, поэтому за пределами проекта удалять нечего. **Реально нечего.** Проверьте сами: `grep -rn "homedir\|os\.home" src/` ничего не вернёт.
...
SafeFlo задуман как безопасная альтернатива функционально близким проектам. Главные архитектурные отличия:
